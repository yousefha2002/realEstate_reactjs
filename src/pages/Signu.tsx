...
function Signu() {
    ...
    const {mutateAsync , isLoading} = useMutation(handleSignup);

    const onSubmit: SubmitHandler<IFormInputs> = (data) => mutateAsync(data)

    async function handleSignup(dataForm:IFormInputs)
    {
        try{
    ...
    <div className='grid grid-cols-12'>
        ...
        <div className=' col-span-12 md:col-span-4'>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='w-[80%] mx-auto my-5'>
                <div className='mb-5'>
                    <InputLabel text='إسم المستخدم'/>
                    <Controller
                        name="name"
                        control={control}
                        rules={{ required: true }}
                        render={({ field }) => <input type="text" placeholder='إسم المستخدم' {...field}
                ...
                </div>
                {
                    ...
                    :
                    <button type='submit' className='w-full bg-primary text-white rounded-[20px] py-2 duration-200 hover:opacity-80'>تسجيل مستخدم جديد</button>
                }
                <h4 className=' text-center text-[14px] mt-4'>لديك حساب؟ <Link className='text-primary' to={"/login"}>تسجيل دخول</Link></h4>
            </form>
        </div>
    </div>
)
